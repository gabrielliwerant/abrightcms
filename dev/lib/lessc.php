...
class lessc {
	...
	// @param boolean $force_run Force compiling regardless (Gabriel Liwerant)
	public static function ccompile($in, $out, $force_run = false) {
		if ((!is_file($out) || filemtime($in) > filemtime($out)) || $force_run) {
			$less = new lessc($in);
